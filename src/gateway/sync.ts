...
import { R2_TENANTS_PATH } from '../config';
...
export async function syncToR2(
  ...
  const mounted = await mountR2Storage(sandbox, env, tenantSlug);
  if (!mounted) {
    return { success: false, error: 'Failed to mount R2 storage' };
  }

  const tenantPath = `${R2_TENANTS_PATH}/${tenantSlug}`;
  ...
  const syncCmd = `mkdir -p ${tenantPath}/clawdbot ${tenantPath}/skills && rsync -r --no-times --delete --exclude='*.lock' --exclude='*.log' --exclude='*.tmp' /root/.clawdbot/ ${tenantPath}/clawdbot/ && rsync -r --no-times --delete /root/clawd/skills/ ${tenantPath}/skills/ && date -Iseconds > ${tenantPath}/.last-sync`;
  ...
  try {
    ...
    const timestampProc = await sandbox.startProcess(`cat ${tenantPath}/.last-sync`);
